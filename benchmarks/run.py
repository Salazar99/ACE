...
# local_rtl: paths relative to this design's own folder, in compile order. Every design
#            folder is self-contained - RTL, testbench, and for some of them a generate.sh
#            that produces the design's ten traces with nothing but verilator, independently
#            of this script.
DESIGNS = {
    "comparator_3bit": {
        "local_rtl": ["rtl/verified_comparator_3bit.v"],
        "cycles": 400,
    },
    "adder_8bit": {
        "local_rtl": ["rtl/verified_adder_8bit.v"],
        "cycles": 500,
    },
    "ibex_csr": {
        "local_rtl": ["rtl/ibex_csr.sv"],
        "cycles": 500,
    },
    "accumulator": {
        "local_rtl": ["rtl/accuintf.sv", "rtl/verified_accu.sv"],
        "cycles": 800,
    },
    "multi_16bit": {
        "local_rtl": ["rtl/verified_multi_16bit.v"],
        "cycles": 1000,
    },
    "sqrt": {
        "local_rtl": ["rtl/sqrtintf.sv", "rtl/sqrt_calculator.sv"],
        "cycles": 1000,
    },
    "ibex_alu": {
        "local_rtl": ["rtl/ibex_pkg.sv", "rtl/ibex_alu.sv"],
        "cycles": 1200,
    },
    "ibex_multdiv_fast": {
        "local_rtl": ["rtl/ibex_pkg.sv", "rtl/ibex_multdiv_fast.sv"],
        "cycles": 1500,
    },
    # protocol blocks written for this benchmark: request/grant arbitration, a queue with
    # flags, and a bus slave with wait states.
    "arbiter4": {
...
def build(design, build_dir):
    spec = DESIGNS[design]
    rtl = [str(BENCH / design / p) for p in spec["local_rtl"]]
...
def main():
    ...
    for design in designs:
        ...
        if not args.validate_only:
            ...
            sim = build(design, Path(args.build_dir))
