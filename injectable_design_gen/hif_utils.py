...
def generate_injectable_design(verilog_files):
    if globals.debug:
        print(globals.CDBG+"DEBUG_MSG"+globals.CEND)
        print("Generating injectable design \n")

    verilog2hif_command = f"verilog2hif {verilog_files} --output {globals.hif_tb_prefix}synthetic_design.hif.xml"
    if globals.debug:
        print(globals.CDBG+"DEBUG_MSG"+globals.CEND)
    ...
    hif2sc_command = f"hif2sc {globals.hif_tb_prefix}synthetic_design.ddt.a2t.muffin.hif.xml --extensions cpp_hpp --directory {globals.hif_tb_prefix}injected"
    if globals.debug:
        print(globals.CDBG+"DEBUG_MSG"+globals.CEND)
...
##############################################
#Run hif tb - entry point for the hif pipeline
##############################################
def run_hifsuite(specification,v_files):
    if globals.debug:
        print(globals.CDBG+"DEBUG_MSG"+globals.CEND)
        print("Performing Fault injection and simulation \n")
   
    #run hif pipeline
    generate_injectable_design(v_files)  
    ...
    try:
        subprocess.run(f"cp {globals.hif_tb_prefix}components/CMakeLists.txt {globals.hif_tb_prefix}injected/", stdout=subprocess.DEVNULL, shell=True, check=True)
    except subprocess.CalledProcessError as e:
        print(globals.CERR +"Error:" + globals.CEND + f"Failed to copy Makefile to {globals.hif_tb_prefix}. {e}")
        exit(1)
    #copy cpptracer to the simulation folder
    try:
        subprocess.run(f"cp -r {globals.hif_tb_prefix}components/cpptracer {globals.hif_tb_prefix}injected/src", stdout=subprocess.DEVNULL, shell=True, check=True)
    except subprocess.CalledProcessError as e:
        print(globals.CERR +"Error:" + globals.CEND + f"Failed to copy cpptracer to {globals.hif_tb_prefix}. {e}")
        exit(1)
    ...
    os.chdir(f"{globals.hif_tb_prefix}injected")
    ...
    try:
        subprocess.run(f"cp {globals.hif_tb_prefix}/injected/traces/*.vcd {globals.out_folder}faulty_traces/vcd", shell=True, check=True)
    except subprocess.CalledProcessError as e:
    ...
    if not globals.debug:
        try:
            subprocess.run(f"rm -rf {globals.hif_tb_prefix}injected {globals.hif_tb_prefix}rtl {globals.hif_tb_prefix}*.xml" , shell=True, check=True)
        except subprocess.CalledProcessError as e:
            print(globals.CERR +"Error:" + globals.CEND + f"Failed to clean up the simulation directory. {e}")
            exit(1)
...
def generate_testbench(specification):
    ...
    with open(globals.hif_tb_prefix + 'injected/src/main.cpp', 'w') as testbench_file:
        testbench_file.write(testbench)

    if globals.debug: 
        print(globals.CDBG+"DEBUG_MSG"+globals.CEND)
        print(f"Generated testbench: {globals.hif_tb_prefix}injected/src/main.cpp \n")
