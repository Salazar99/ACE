...
def process_csv(file_path):
    ...
    for column, bit_size in uint_columns:
        if bit_size == 64:
            df[column] = df[column].apply(lambda x: uint_to_signed(int(x), bit_size)).astype(np.int64)
        else:
            df[column] = df[column].apply(lambda x: uint_to_signed(int(x), bit_size)).astype(np.int32)

        new_column_name = column.replace("uint32_t", "int").replace("uint64_t", "int").replace("unsigned char", "int")
        df.rename(columns={column: new_column_name}, inplace=True)
