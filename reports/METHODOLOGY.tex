...
247 completions.\footnote{\texttt{ace/\_\_main\_\_.py:50--61}.} Signals that are already
...
taking the deduplicated antecedents of what comes back.\footnote{\texttt{ace/\_\_main\_\_.py:64--89};
grammars \texttt{G1}--\texttt{G5} in \texttt{ace/backends.py:43--59}, default \texttt{G3}
...
so that the numbers reported for a HARM run and for a run without HARM are the same
quantity. When no miner is installed the flow falls back to an in-process vocabulary built
from the observed values, the interval bounds, the three orderings $a>b$, $a=b$, $a<b$ for
every input pair and their pairwise conjunctions.\footnote{\texttt{ace/\_\_main\_\_.py:92--116}.}
The fallback is a comparison rather than a dependency: what a real miner adds is its own
vocabulary construction and ranking.
...
member of the other and conversely.\footnote{\texttt{ace/\_\_main\_\_.py:125--171}, reusing the
