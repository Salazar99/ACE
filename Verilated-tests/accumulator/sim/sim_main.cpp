...
static inline void eval_and_dump(Vverified_accu * top, VerilatedVcdC* tfp) {
    top->eval();
    if (tfp) tfp->dump(sim_time);
}

static inline void step_half_cycle(Vverified_accu * top, VerilatedVcdC* tfp) {
    eval_and_dump(top, tfp);
    sim_time += HALF_PERIOD;
    top->clk = !top->clk;
}

static inline void step_full_cycle(Vverified_accu * top, VerilatedVcdC* tfp) {
...
int main(int argc, char **argv)
{
    ...
    while (cycles < 1000000)
    {
        ...
        uint8_t rand_data = std::rand() & 0xFF;
        top->data_in = rand_data;
        top->valid_in = 1;
        for (int i = 0; i < 15; ++i) {
