...
#ifndef VERILATED_VIBEX_ALU_H_
#define VERILATED_VIBEX_ALU_H_  // guard

#include "verilated.h"

class Vibex_alu__Syms;
class Vibex_alu___024root;
class VerilatedVcdC;
class Vibex_alu_ibex_pkg;


// This class is the main interface to the Verilated model
class alignas(VL_CACHE_LINE_BYTES) Vibex_alu VL_NOT_FINAL : public VerilatedModel {
  private:
  ...
  public:
    ...
    Vibex_alu_ibex_pkg* const __PVT__ibex_pkg;

    // Root instance pointer to allow access to model internals,
    // including inlined /* verilator public_flat_* */ items.
    Vibex_alu___024root* const rootp;
    ...
    explicit Vibex_alu(VerilatedContext* contextp, const char* name = "TOP");
    explicit Vibex_alu(const char* name = "TOP");
    /// Destroy the model; called (often implicitly) by application code
    virtual ~Vibex_alu();
  private:
    VL_UNCOPYABLE(Vibex_alu);  ///< Copying not allowed
